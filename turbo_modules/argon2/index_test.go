...
func TestParseOptions(t *testing.T) {
	...
	customOpts := rt.NewObject()
	_ = customOpts.Set("memoryCost", 128)
	_ = customOpts.Set("timeCost", 2)
	_ = customOpts.Set("variant", "argon2i")

	args2 := []goja.Value{rt.ToValue("password"), customOpts}
...
func TestSynchronousFunctions(t *testing.T) {
	...
	// Test verifySync
	verifyScript := `
		const isValid = argon2.verifySync('` + hash + `', 'test-password');
		isValid;
	`
	...
	// Test with wrong password
	wrongScript := `
		const isValid = argon2.verifySync('` + hash + `', 'wrong-password');
		isValid;
	`
